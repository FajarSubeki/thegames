...
class GameDetailsBloc extends Bloc<GameDetailsEvent, GameDetailsState> {
  ...
  GameDetailsBloc(this.getGameDetailsUseCase)
      : super(GameDetailsInitialState()) {
    on<GetGameDetailsEvent>(
      getGameDetailsEventObserver,
    );
  }

  Future<void> getGameDetailsEventObserver(
      GetGameDetailsEvent event, Emitter<GameDetailsState> emit) async {
    emit(GameDetailsLoadingState());
    final result = await getGameDetailsUseCase(GameDetailsParams(id: event.id));
    result.fold(
      (failure) =>
          emit(GameDetailsErrorState(message: mapFailureToMessage(failure))),
      (gameDetails) {
